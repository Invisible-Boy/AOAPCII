...
#include <iomanip>
using namespace std;
//这个版本测试，分开处理奖金和集中计算奖金的精度是否会影响OJ评判，结果是不会
        ...
        for(int i = 0; i < (int)p_base.size();)
        {
            ...
            for(int j = 0; j < temp; j++)
            {
                ...
                if(p_base[i + j].dq == 4)
                {
                    ...
                    cout << setw(10) << p_base[i + j].total4 << '$' << right << setw(9) << setiosflags(ios::fixed) <<setprecision(2) << money << endl;
                }
                else
                    cout << "DQ" << endl;
            }
