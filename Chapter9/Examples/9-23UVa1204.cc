...
int pi[105];
bool kmp(const string &lhs, const string &rhs) {
    int n = lhs.length(), m = rhs.length();
    int now = 0;
    for(int i = 2; i <= n; i++) {
        while(lhs[i - 1] != lhs[now] && now)
            now = pi[now];
        now += lhs[i] == lhs[now] ? 1 : 0;
        pi[i] = now;
    }
    now = 0;
    for(int i = 0; i < m; i++) {
        while(rhs[i] != lhs[now] && now)
            now = pi[now];
        now += (rhs[i] == lhs[now] ? 1 : 0);
